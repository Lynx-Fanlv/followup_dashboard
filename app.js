...
function addToPending(files) {
  ...
  renderPending();
  // 用户已经开始动手用了：收起「第一次使用？」引导气泡，别挡着
  if (added) tutHideCoach(false);
...
async function doSnapshot(desen) {
  ...
  try {
    ...
    // 教程入口按钮在快照里本来就是隐藏的，连「未看过」的呼吸/红点标记也一并摘掉，
    // 让快照完全不含任何教程痕迹（否则万一样式被覆盖，接收方会看到一个闪烁的死按钮）。
    const tutBtnEl = $("#tutBtn");
    const tutBtnNew = !!(tutBtnEl && tutBtnEl.classList.contains("is-new"));
    if (tutBtnNew) tutBtnEl.classList.remove("is-new");
...
function loadSnapshot(snap) {
  ...
  tutHideCoach(false);   // 快照是只读分享件，不该出现「第一次使用？」引导气泡
...
const TUT = { on: false, i: 0, steps: [], target: null, snap: null, injected: false,
              demoState: "", coach: null, coachTimer: 0,
...
function tutSteps() {
  ...
  // 从根上杜绝「整屏只有上传框、看不到任何功能」的空教程。
  return steps.map((s, k) => (k >= 1 ? Object.assign({ needData: true }, s) : s));
}
...
async function tutGo(i) {
  ...
  // 自愈：这一步要在「有数据的看板」上讲解，但当前一条记录都没有 —— 先补上内置示例数据，
  // 否则聚光灯会框在一个空的看板上（这正是此前教程看起来"只有上传按钮"的原因）。
  if (st.needData && !STORE.records.length) {
    try { await tutEnsureDemoData(); } catch (e) { console.warn("示例数据准备失败", e); }
    try { TUT.steps = tutSteps(); } catch (_) {}   // 重新求值，让文案带上最新数据量
  }
  ...
  c.querySelector(".tut-body").innerHTML =
    (typeof st.body === "function" ? st.body() : (st.body || ""));
  const acts = c.querySelector(".tut-acts");
  acts.innerHTML = "";
  const actList = (typeof st.acts === "function" ? st.acts() : st.acts) || [];
  if (actList.length) {
...
// 生成并解析内置示例数据。返回是否成功（不再弹 alert：教程里自动加载时打断体验，
// 失败由调用方在说明卡里以「重试」按钮呈现）。
...
// 教程必须「有数据」才有内容可讲：没有任何数据时自动补上内置示例数据。
// 用户自己已经加载过数据时**绝不注入**（教程直接跑在他的真实数据上）。
// 返回值写入 TUT.demoState，供说明卡动态渲染：ready=已加载示例 / user=用户自有数据 / fail=失败
async function tutEnsureDemoData() {
  if (STORE.records.length) return (TUT.demoState = "user");
  if (TUT.injected) return (TUT.demoState = "ready");
  try {
    await tutLoadDemo();
    TUT.demoState = TUT.injected ? "ready" : "fail";
  } catch (e) {
    console.warn("示例数据加载失败", e);
    TUT.demoState = "fail";
  }
  if (TUT.demoState === "fail") console.warn("教程示例数据不可用，将退化为无数据的讲解");
  return TUT.demoState;
}
...
async function tutStart() {
  tutEnsureDom();
  closeAllPopovers();
  tutHideCoach(true);       // 教程一开始就收起引导气泡，并记为「已看过」，不再重复打扰
  TUT.snap = tutSnapshotState();
  TUT.injected = false;
  // 已有数据 → 教程直接跑在用户自己的数据上（绝不注入示例数据）；没有数据 → 进入第 2 步时自动补上
  TUT.demoState = STORE.records.length ? "user" : "";
  TUT.steps = tutSteps();
  TUT.on = true;
  await tutGo(0);
}
...
/* ===== 教程入口的「可发现性」：首次访问引导气泡（coach mark） =====
   按钮上只写「使用方法教程」，用户仍不知道点了会发生什么。按成熟的 onboarding 做法：
   1) 按钮本身换成「▶ 播放图标 + 时长副标签」的通用语汇（▶ = 开始一段引导）；
   2) 首次访问且还没有数据时，在按钮下方弹一个锚定小气泡，写清楚点击后会怎样，
      给「开始教程 / 以后再说」两个出口；看过一次后写入 localStorage，不再打扰。 */
const TUT_SEEN_KEY = "followup_dashboard_tut_seen_v1";
function lsHas(k) { try { return !!localStorage.getItem(k); } catch (_) { return false; } }
function lsSet(k, v) { try { localStorage.setItem(k, v); } catch (_) {} }
function markTutSeen() {
  lsSet(TUT_SEEN_KEY, "1");
  const btn = $("#tutBtn");
  if (btn) btn.classList.remove("is-new");
}

function tutBuildCoach() {
  if (TUT.coach) return TUT.coach;
  const el = document.createElement("div");
  el.className = "tut-coach hidden";
  ...
  el.querySelector(".tut-coach-go").onclick = () => { tutHideCoach(true); tutStart(); };
  el.querySelector(".tut-coach-later").onclick = () => { tutHideCoach(true); markTutSeen(); };
  TUT.coach = el;
  return el;
}

function tutPlaceCoach() {
  const btn = $("#tutBtn"), el = TUT.coach;
  if (!btn || !el) return;
...
// sticky=true 表示「用户已经知道这个入口了」，顺带记为已看过
function tutHideCoach(sticky) {
  if (TUT.coachTimer) { clearTimeout(TUT.coachTimer); TUT.coachTimer = 0; }
  if (TUT.coach) { TUT.coach.classList.remove("on"); TUT.coach.classList.add("hidden"); }
  if (sticky) markTutSeen();
}

function tutMaybePrompt() {
  TUT.coachTimer = 0;
  // 只在「第一次来、还没有数据、不在快照里」时提示；已经在用工具的人不打扰
  if (SNAP_MODE || TUT.on || lsHas(TUT_SEEN_KEY) || STORE.records.length) return;
  const btn = $("#tutBtn");
  if (!btn || btn.classList.contains("hidden")) return;
  btn.classList.add("is-new");
  const el = tutBuildCoach();
  tutPlaceCoach();
  el.classList.remove("hidden");
  requestAnimationFrame(() => el.classList.add("on"));
}

function scheduleCoachPrompt() {
  if (TUT.coachTimer) return;
  TUT.coachTimer = setTimeout(tutMaybePrompt, 900);
}
if (document.readyState === "complete") scheduleCoachPrompt();
else window.addEventListener("load", scheduleCoachPrompt);

$("#tutBtn").onclick = () => { if (TUT.on) { tutEnd("close"); } else { tutStart(); } };
window.addEventListener("resize", () => {
  if (TUT.coach && !TUT.coach.classList.contains("hidden")) tutPlaceCoach();
});
document.addEventListener("keydown", e => {
  if (!TUT.on) {
    if (e.key === "Escape" && TUT.coach && !TUT.coach.classList.contains("hidden")) tutHideCoach(true);
    return;
  }
