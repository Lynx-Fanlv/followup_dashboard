...
async function doSnapshot(desen) {
  ...
  try {
    ...
    // 转义 < 避免记录文本中的 </script> 或 < 破坏内联数据脚本
    const dataJson = JSON.stringify(snap).replace(/</g, "\\u003c");
    const dataScript = `<script>window.__SNAP__=${dataJson};<\/script>`;
    const bootstrap = `<script>(function(){if(window.__SNAP__&&window.AppCore){window.AppCore.loadSnapshot(window.__SNAP__);}})();<\/script>`;
    ...
    html = html.replace("</body>", dataScript + bootstrap + "\n</body>");
...
$("#snapshotPlainBtn").onclick = () => doSnapshot(false);
