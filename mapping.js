...
// 仅覆盖用户确认的两个目标字段：备注、停药/减量根本原因（数组顺序即匹配优先级）。
const CUSTOM_FOLLOWUP_RULES = [
  ["remarks", [
    "患者反馈", "反馈内容", "随访概述", "随访汇总", "情况备注", "备注",
    "最大的顾虑", "顾虑", "希望得到哪些方面", "需求",
  ]],
  ["stop_reduce_reason", [
    "脱落原因", "未复购原因", "当期未复购", "停药原因", "减量原因",
    "停止用药", "推迟用药", "停减量",
  ]],
];
...
// 返回 { remarks: [...], stop_reduce_reason: [...] }，值均为回答原文（不含题目前缀）。
function customExtract(raw) {
  const out = { remarks: [], stop_reduce_reason: [] };
  const items = parseCustomFollowup(raw);
  if (!items.length) return out;
  for (const it of items) {
    if (isPlaceholder(it.value)) continue;
    for (const [field, kws] of CUSTOM_FOLLOWUP_RULES) {
      if (!out[field]) continue;
      if (kws.some(kw => it.name.includes(kw))) {
...
if (typeof window !== "undefined") {
  window.Mapping = { CANONICAL_FIELDS, STATUS_TAXONOMY, SUBTYPE_TAXONOMY, SIGNATURES,
    ...
    CUSTOM_FOLLOWUP_RULES, parseCustomFollowup, customExtract,
